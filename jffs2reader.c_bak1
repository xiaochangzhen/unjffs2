...
int common_write_file(const char *path, char *buf, int len)
{
    FILE *pf = NULL;
    pf = fopen(path, "w+");
...
void unjffs2(const char *raw_path, char *dist_path, char *buffer, size_t size);

void do_unjffs2(char *o, size_t size, struct dir *d, const char *path, const char *dist_path)
{
	struct jffs2_raw_inode *ri;
	size_t rsize;
	char *scratch;
	...
	while (d != NULL) {
		...
		if (d->type == DT_REG) {
			char *tmp;
			tmp = xmalloc(BUFSIZ);
			sprintf(tmp, "%s/%s", dist_path, d->name);
			putblock(scratch, SCRATCH_SIZE, &rsize, ri);
			common_write_file(tmp, scratch, rsize);
			free(tmp);
		} else if (d->type == DT_DIR) {
			char *tmp, *dist_tmp;
			tmp = xmalloc(BUFSIZ);
			dist_tmp = xmalloc(BUFSIZ);
			sprintf(tmp, "%s/%s", path, d->name);
			sprintf(dist_tmp, "%s/%s", dist_path, d->name);
			unjffs2(tmp, dist_tmp, o, size);	/* Go recursive */
			free(tmp);
			free(dist_tmp);
		}
...
void unjffs2(const char *raw_path, char *dist_path, char *buffer, size_t size)
{
	...
	if ( (dir = opendir(dist_path)) ) {
		closedir(dir);
	} else {
		mkdir(dist_path, 0755);	
	}

	dd = resolvepath(buffer, size, 1, raw_path, &ino);

	if (ino == 0 ||
			(dd == NULL && ino == 0) || (dd != NULL && dd->type != DT_DIR))
		errmsg_die("%s: No such file or directory", raw_path);

	d = collectdir(buffer, size, ino, d);
	do_unjffs2(buffer, size, d, raw_path,  dist_path);
	freedir(d);
...
int main(int argc, char **argv)
{
	...
	char *scratch, *dir = NULL, *file = NULL, *output = NULL;
	size_t ssize = 0;

	char *buf;

	while ((opt = getopt(argc, argv, "rd:f:to:")) > 0) {
		switch (opt) {
			case 'd':
				dir = optarg;
				break;
